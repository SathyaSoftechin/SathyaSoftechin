      ...
      <div className="relative max-w-7xl mx-auto h-[88px] px-6 flex items-center">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 hidden md:block">
          ...
          >
            ...
            {showPill && (
              <span
                className="absolute left-0 top-1/2 rounded-full border border-orange-400 shadow-md shadow-orange-400 transition-all duration-300 ease-out"
                style={pillStyle}
              />
            )}
            ...
            {navLinks.map((link) => (
              <NavLink
                key={link.name}
                to={link.path}
                className="relative z-10 px-6 py-2 text-sm font-medium text-black"
              >
                {link.name}
              </NavLink>
            ))}
